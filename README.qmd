...
This analysis follows a _Nested Modeltime Workflow_ from **`modeltime`** along with using the **`NNS`** package. I use this to monitor the downloads of all of my packages:
...
`r dl_birth_datetime()`, and at report knit time is `r hours_since_cran_log_update() * -1` 
...
ts_downloads_tbl(data_tbl, "week", package) |>
  group_split(package) |>
  imap(
    .f = function(obj, id){
      obj |>
      ...
      geom_line() +
      geom_vline(
        data = filter(pkg_tbl, package == obj$package[1])
        , aes(xintercept = date, color = package)
        , lwd = 1
        , lty = "solid"
      ) +
...
```{r ts_decomp_seasonal_plt, echo=FALSE}
ts_downloads_tbl(
    .data = data_tbl,
    .by_time = "day",
    package
    ...
    map(\(x) x |>    
        ...
        theme_minimal() +
        labs(
            title = "Seasonal Diagnositcs: log1p(Values)",
            subtitle = x$package
            )
    )
```
...
```{r ts_decomp_acf_plt, echo=FALSE}
ts_downloads_tbl(
    .data = data_tbl,
    .by_time = "day",
    package
